...
| `conv2d` | `conv1x1`, `conv3x3_relu` |
| `maxpool2d` | `maxpool2d_stride2`, `maxpool2d_stride2_tail`, `maxpool2d_window`, `maxpool2d_nms9`, `maxpool2d_nms7` |
| `l2_normalize` | `l2_normalize_channels` |
| `depth_to_space` | `depth_to_space` |
| `channel_peak` | `channel_peak` |
| `score_threshold` | `score_threshold` |

Every kernel above is wide-vector FP32 only (`wide_vector_debug=True`): they
build on `exp2`/`reciprocal`/`rsqrt` over an FP32 vector path, or on the
512-byte row granularity that mode gives, and none has a narrow (INT8/FP8)
variant. This is reported as a caveat on every verdict.

Which kernel runs each operation of a real network -- and which operations have
none, and why -- is on the [SuperPoint layer map](kernels/superpoint.md).
