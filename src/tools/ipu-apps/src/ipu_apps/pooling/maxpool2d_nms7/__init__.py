"""7x7 stride-1 centred max-pool, fully unrolled (FP32 wide-vector mode).

The bespoke twin of :mod:`~ipu_apps.pooling.maxpool2d_window`, fixed at
``K = 7`` -- SuperPoint's ``simple_nms`` window at
``nms_radius = 3``, a tighter suppression than the default. Identical result,
28% fewer cycles.
...
======================  ==========================  ==================
row load latency        1 dead word per row (7)     folded into the taps
...
======================  ==========================  ==================
    ...
    per output tile   K^2 + 3K + 5 = 75    ->   K^2 + 5 = 54 words
    480x640, 1 plane        217,447        ->   ~157,000 cycles

The assembled program is 64 words, inside the 128-word IMEM bank.
...
Everything else -- the halo tiling (``TC = 122``), the ``-FLT_MAX`` border, the
memory layout -- is ``maxpool2d_window``'s, unchanged, so the two are
interchangeable and the registry picks this one on cost at ``K = 7``.

Usage::

    from ipu_apps.pooling.maxpool2d_nms7 import MaxPool2dNms7App

    app = MaxPool2dNms7App(
        inst_path="maxpool2d_nms7.bin",
...
def _caveats(**params):
    ...
    notes = [
        WIDE_VECTOR_ONLY,
        "K is fixed at 7 by the unroll: this kernel is SuperPoint's "
        "nms_radius=3 window and cannot be asked for another size.",
    ]
...
SPEC = KernelSpec(
    ...
    op="maxpool2d",
    variant="nms7",
    app_class=MaxPool2dNms7App,
    asm="maxpool2d_nms7.asm",
