{#- ==========================================================================
    maxpool2d_nms9.asm -- 9x9 stride-1 centred max, FULLY UNROLLED

    out[c, y, x] = max over dy, dx in [0, 9) of pad(in)[c, y+dy-4, x+dx-4]

    SuperPoint's simple_nms window at its default nms_radius = 4. This is the
    bespoke twin of maxpool2d_window: same result, K fixed at 9 and the taps
    ...
    3. NO GUARD ROW. A rolling prefetch would read one row past the plane on
       ...
       load.
        ...
        per output tile:   K^2 + 3K + 5 = 113 words   ->   K^2 + 5 = 86
        480x640, 1 plane:        326,887 cycles       ->   ~249,000  (-24%)
    ...
    HALO TILING is maxpool2d_window's, unchanged: TC = 128-8 = 120 output
    columns per row, element e of tile t is input column t*120 + e - 4, and the
    largest element a valid lane reads is 119+8 = 127. The P=4 border rows and
    ...
    CR map (set by the harness; CR0/CR1 are READ-ONLY hardware constants):
      ...
      CR6  = IN_PLANE_STRIDE = (H + 8) * TILES_PER_ROW
